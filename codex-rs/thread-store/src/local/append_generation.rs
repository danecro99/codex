...
struct SourcePosition {
    ...
    /// Whether the physical container was the compressed sibling when this position was taken.
    #[serde(default)]
    compressed: bool,
...
pub(super) fn bootstrap_current(
    ...
    let canonical_rollout_path = canonical_logical_path(rollout_path)?;
...
fn positions_equal(left: &SourcePosition, right: &SourcePosition) -> bool {
    if left.compressed != right.compressed {
        // Rollout compression rewrites the container without changing a single logical byte, so
        // the OS-level identity fence is re-anchored while the logical prefix must still match
        // exactly. A compressed rollout is closed for appends, so no append can hide here.
        return left.end_byte_offset == right.end_byte_offset
...
fn source_position(
    ...
    // Durable identity comes from the physical container; every offset addresses logical JSONL
    // bytes, so a compressed rollout is inspected without being rewritten as plain.
    let physical = resolve_physical_source(path)?;
    let before = std::fs::metadata(physical.path.as_path()).map_err(source_error)?;
    let before_modified = before.modified().map_err(source_error)?;
    let before_generation = platform_file_generation(&before)?;
    let end_byte_offset = if physical.compressed {
        codex_rollout::rollout_logical_len(path).map_err(source_error)?
    } else {
        before.len()
    };
    ...
    let after = std::fs::metadata(physical.path.as_path()).map_err(source_error)?;
    let after_modified = after.modified().map_err(source_error)?;
    ...
    Ok(SourceInspection {
        position: SourcePosition {
            ...
            modified_unix_nanos: modified_unix_nanos(after_modified)?,
            file_identity,
            change_marker,
...
fn terminal_ordinal_exclusive(
    ...
    match history_mode {
        ...
        ThreadHistoryMode::Paginated => {
            let terminal = read_terminal_rollout_line(path, end_byte_offset)
                .map_err(|message| {
                    invalid(format!("source terminal record is unusable: {message}"))
                })?
                .ok_or_else(|| invalid("source rollout is empty"))?;
...
pub(super) struct TerminalRolloutLine {
    pub(super) line: RolloutLine,
...
fn summarize_suffix(
    ...
    let mut file = codex_rollout::open_rollout_seekable_reader(path).map_err(source_error)?;
...
fn load_journal(
    ...
    File::open(path.as_path())
        .map_err(|err| invalid(format!("cannot open append generation: {err}")))?
        .take(MAX_APPEND_GENERATION_BYTES.saturating_add(1))
...
/// Physical file backing a logical rollout path.
///
/// Offsets, hashes, and fences all address the original JSONL bytes, but durable file metadata -
/// length, modification time, and platform generation - belongs to whichever container actually
/// exists. Compressed rollouts keep their logical `.jsonl` identity and are never materialized.
pub(super) struct PhysicalSourceFile {
    pub(super) path: PathBuf,
    pub(super) compressed: bool,
}

/// Resolves the physical rollout container for a logical JSONL path.
pub(super) fn resolve_physical_source(
    logical_path: &Path,
) -> ThreadStoreResult<PhysicalSourceFile> {
    let plain = codex_rollout::plain_rollout_path(logical_path);
    if plain
        .try_exists()
        .map_err(|err| source_probe_error(plain.as_path(), err))?
    {
        return Ok(PhysicalSourceFile {
            path: plain,
            compressed: false,
        });
    }
    ...
    if compressed
        .try_exists()
        .map_err(|err| source_probe_error(compressed.as_path(), err))?
    {
        return Ok(PhysicalSourceFile {
            path: compressed,
            compressed: true,
    ...
    Err(ThreadStoreError::InvalidRequest {
        message: format!("rollout source {} does not exist", plain.display()),
    })
}

fn source_probe_error(path: &Path, err: io::Error) -> ThreadStoreError {
    ThreadStoreError::Internal {
        message: format!("failed to probe rollout source {}: {err}", path.display()),
    }
}

/// Canonicalizes a logical rollout path without requiring its plain representation to exist.
///
/// The physical file may be the compressed sibling, so the directory is canonicalized and the
/// logical `.jsonl` file name is reattached.
pub(super) fn canonical_logical_path(path: &Path) -> ThreadStoreResult<PathBuf> {
    let logical = codex_rollout::plain_rollout_path(path);
    let (Some(parent), Some(file_name)) = (logical.parent(), logical.file_name()) else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
                "rollout source path has no file name: {}",
                logical.display()
            ),
        });
    };
    let file_name = file_name.to_os_string();
    // Prove the file exists in one of its representations before pinning its logical identity.
    let _ = resolve_physical_source(logical.as_path())?;
    let canonical_parent =
        std::fs::canonicalize(parent).map_err(|err| ThreadStoreError::Internal {
            message: format!(
                "failed to canonicalize rollout source directory {}: {err}",
                parent.display()
            ),
        })?;
