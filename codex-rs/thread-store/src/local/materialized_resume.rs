...
const CHECKPOINT_DIRECTORY: &str = "materialized_resume_state_v5";
...
pub(super) async fn capture_current_source(
    ...
    let path = codex_rollout::plain_rollout_path(rollout_path);
    let canonical_rollout_path = super::append_generation::canonical_logical_path(path.as_path())?;
    ...
    let physical_source_is_compressed =
        super::append_generation::resolve_physical_source(canonical_rollout_path.as_path())?
            .compressed;
    let (
        end_byte_offset,
    ...
    let lineage = match history_mode {
        ...
        ThreadHistoryMode::Paginated => {
            ...
            for segment in resolved.segments() {
                ...
                let canonical_segment_path = super::append_generation::canonical_logical_path(
    ...
    let verified_physical =
        super::append_generation::resolve_physical_source(canonical_rollout_path.as_path())?;
    let verified =
        std::fs::metadata(verified_physical.path.as_path()).map_err(source_metadata_error)?;
    if verified_physical.compressed != physical_source_is_compressed
...
/// Re-verifies that a captured source still sits exactly on its durable fence.
///
/// Replays that cannot be byte-fenced while they read - the Legacy complete-replay path - use
/// this after the read so a source that grew underneath them is rejected instead of being paired
/// with a fence it no longer matches.
pub(super) fn verify_source_unchanged(source: &MaterializedResumeSource) -> ThreadStoreResult<()> {
    let path = source.canonical_rollout_path.as_path();
    let physical = super::append_generation::resolve_physical_source(path)?;
    let verified = std::fs::metadata(physical.path.as_path()).map_err(source_metadata_error)?;
    if modified_unix_nanos(verified.modified().map_err(source_metadata_error)?)?
        != source.modified_unix_nanos
    {
        return Err(ThreadStoreError::Conflict {
            message: "resume source changed while its durable replay was read".to_string(),
        });
    }
    // Length and mtime alone cannot see a same-size rewrite, so re-check the same bounded head and
    // tail samples that fenced the capture.
    let (prefix_head_sha256, _) = hash_sample(path, 0, source.end_byte_offset)?;
    let tail_start = source
        .end_byte_offset
        .saturating_sub(FENCE_SAMPLE_BYTES as u64);
    let (prefix_tail_sha256, _) = hash_sample(path, tail_start, source.end_byte_offset)?;
    if prefix_head_sha256 != source.prefix_head_sha256
        || prefix_tail_sha256 != source.prefix_tail_sha256
    {
        return Err(ThreadStoreError::Conflict {
            message: "resume source was rewritten while its durable replay was read".to_string(),
        });
    }
    Ok(())
}
...
fn read_existing_artifact(
    ...
    File::open(path.as_path())
        .map_err(|err| invalid_checkpoint(format!("cannot open artifact: {err}")))?
        .take(MATERIALIZED_RESUME_STATE_HARD_MAX_BYTES.saturating_add(1))
        .read_to_end(&mut bytes)
        .map_err(|err| invalid_checkpoint(format!("cannot read artifact: {err}")))?;
...
fn stable_position(
    ...
    // Durable metadata lives on the physical container; every offset addresses the logical JSONL
    // bytes, so a compressed source is fenced without being rewritten as plain.
    let physical = super::append_generation::resolve_physical_source(path)?;
    let before = std::fs::metadata(physical.path.as_path()).map_err(source_metadata_error)?;
    ...
    let end_byte_offset = if physical.compressed {
        codex_rollout::rollout_logical_len(path).map_err(source_metadata_error)?
    } else {
        before.len()
    };
    let (end_ordinal_exclusive, terminal_scan_bytes, terminal_scan_items) = match history_mode {
        ...
        ThreadHistoryMode::Paginated => {
            let terminal =
                super::append_generation::read_terminal_rollout_line(path, end_byte_offset)
                    .map_err(|message| {
                        invalid_checkpoint(format!(
                            "source terminal record cannot be fenced: {message}"
                        ))
                    })?
                    .ok_or_else(|| invalid_checkpoint("source rollout is empty"))?;
            let ordinal = terminal.line.ordinal.ok_or_else(|| {
            ...
            (Some(end_ordinal_exclusive), terminal.bytes_read, 1)
        }
    };
    let after = std::fs::metadata(physical.path.as_path()).map_err(source_metadata_error)?;
...
fn hash_sample(path: &Path, start: u64, end: u64) -> ThreadStoreResult<(String, u64)> {
    ...
    // Offsets always address the original JSONL bytes; the seekable reader decodes a compressed
    // rollout into a bounded anonymous snapshot instead of hashing its container bytes.
    let mut file =
        codex_rollout::open_rollout_seekable_reader(path).map_err(source_metadata_error)?;
