...
pub(super) async fn update_thread_metadata(
    ...
    let history_mode = if has_explicit_metadata {
        match live_writer::live_writer_parts(store, thread_id).await {
            Ok(writer) => Some(writer.history_mode),
...
async fn append_item_with_generation(
    ...
    // The fence is history-mode specific: a paginated source is fenced on its terminal ordinal,
    // so inspecting it as Legacy would report a position the journal can never match.
